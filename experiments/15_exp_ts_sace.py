...
from ece.pooler import TsIdentity
from ece.pooler import TsPooler
from ece.pooler import TsAutoencoder
...
def experiment(bb, X_train, X_test, y_train, y_test, pooler_type, window_size,
    ...
    if pooler_type == 'identity':
        ...
        latent_dim = 4
        ae_type = 'vae'
        path_ae_local = path_ae + '/%s_ts_1000/' % ae_type
        name = '%s_%s' % (dataset, ae_type)
        pooler = TsAutoencoder(input_shape, latent_dim, n_classes, ae_type, path_ae_local, name)
    ...
    print(datetime.datetime.now(), dataset, black_box, 'ens-h', 'ts', pooler_type)
...
def main():
    ...
    for pooler_type in [
        ...
        filename_results = path_results + 'ts_ens_%s.csv' % pooler_type
