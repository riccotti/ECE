...
def experiment(bb, bb_keras, X_train, X_test, y_train, y_test, window_size,
    ...
    ae = load_model(path_ae + 'cegp_ae_%s.h5' % dataset)
    enc = load_model(path_ae + 'cegp_ae_%s_enc.h5' % dataset)

    # initialize CEM explainer and explain instance
    exp = CounterFactualProto(predict_fn, shape, gamma=gamma, theta=theta,
    ...
    print(datetime.datetime.now(), dataset, black_box, 'cegp', 'img')
    ...
    for test_id, i in enumerate(index_test_instances):
        ...
        print(datetime.datetime.now(), dataset, black_box, 'cegp',
...
def main():
    ...
    path_ae_local = path_ae + 'cegp_ae_img_4/'

    filename_results = path_results + 'img_cegp.csv'
